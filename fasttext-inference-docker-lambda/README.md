## Serverless fasttext Model Serving

This examples illustrates how to serve fasttext model on Lambda Function to predict based on dbpedia dataset, with a BlazingText model.

This project contains source code and supporting files for a serverless application that you can deploy with the SAM CLI. It includes the following files and folders.

- app - Code for the application's Lambda function.
- train-code - Code for training fasttext model based on iris dataset.
...
```bash
fasttext-inference-docker-lambda$ sam build
```
...
```bash
fasttext-inference-docker-lambda$ sam local invoke FastTextInferenceFunction --event events/event.json
```
...
```bash
fasttext-inference-docker-lambda$ sam logs -n FastTextInferenceFunction --stack-name fasttext-inference-docker-lambda --tail
```
...
```bash
aws cloudformation delete-stack --stack-name fasttext-inference-docker-lambda
```
