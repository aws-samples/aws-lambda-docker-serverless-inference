...
The application illustrates how to serve XGBoost model to predict breast cancer.
...
```bash
xgboost-inference-docker-lambda$ sam build
```
...
```bash
xgboost-inference-docker-lambda$ sam local invoke XGBoostInferenceFunction --event events/event.json
```
...
```bash
xgboost-inference-docker-lambda$ sam logs -n XGBoostInferenceFunction --stack-name xgboost-inference-docker-lambda --tail
```
...
```bash
aws cloudformation delete-stack --stack-name xgboost-inference-docker-lambda
```
